...
// Bulk transfer: all products listed, enter qty to move from main → pharmacy.
// Rows with 0 skipped. Validates against main stock before submit.
...
export function TransferClient({ products }: { products: Product[] }) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (overStock.length > 0) {
      toast.error(`Insufficient main stock: ${overStock.map((p) => p.genericName).join(", ")}`);
      return;
    }
    ...
    startTransition(async () => {
      ...
      for (const product of toTransfer) {
        ...
        else failed.push(product.genericName);
      }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-xl font-semibold">Transfer — Main Stock → Pharmacy</h2>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/40">
                  ...
                  <th className="px-3 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase w-28">Main Stock</th>
                  <th className="px-3 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase w-28 hidden sm:table-cell">Pharmacy</th>
                  <th className="px-3 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase w-28">Transfer Qty</th>
                  <th className="px-3 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase w-28 hidden md:table-cell">Batch</th>
                </tr>
              </thead>
              <tbody>
                    ...
                    <tr key={product.id} className={`border-b last:border-0 transition-colors ${over ? "bg-destructive/5" : hasQty ? "bg-primary/5" : "hover:bg-muted/20"}`}>
                      ...
                      <td className="px-3 py-2">
                        <span className={`text-sm font-medium tabular-nums ${product.mainStock === 0 ? "text-destructive" : ""}`}>
                          {product.mainStock}<span className="ml-0.5 text-xs text-muted-foreground font-normal">{product.unit}</span>
                        </span>
                      </td>
                      <td className="px-3 py-2 hidden sm:table-cell">
                        <span className="text-sm tabular-nums text-muted-foreground">{product.pharmacyStock} {product.unit}</span>
                      </td>
                      <td className="px-3 py-2">
