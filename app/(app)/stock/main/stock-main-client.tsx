            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/40">
                  ...
                  <th className="px-2 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase w-24 hidden md:table-cell">Price</th>
                  <th className="px-2 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase w-24 hidden lg:table-cell">Batch</th>
                  <th className="px-2 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase w-28 hidden lg:table-cell">Expiry</th>
                  <th className="px-2 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase w-28 hidden xl:table-cell">Supplier</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((product, idx) => {
                  ...
                  return (
                    <tr key={product.id} className={`border-b last:border-0 transition-colors ${hasQty ? "bg-primary/5" : "hover:bg-muted/20"}`}>
                      ...
                      {/* Product — brand only on mobile, generic shown on sm+ */}
                      ...
                      <td className="px-2 py-2 whitespace-nowrap">
                        <span className={`text-xs font-medium tabular-nums ${
                          status === "out" ? "text-destructive" : status !== "normal" ? "text-warning" : ""
                        }`}>
                          {product.currentStock}
                          <span className="ml-0.5 text-muted-foreground font-normal">{product.unit}</span>
                        </span>
                      </td>
                      {/* Qty input */}
                      <td className="px-2 py-2">
                        <input type="number" min="0" value={row?.quantity ?? ""} placeholder="0"
                          ...
                          data-main-qty
                          className={`h-8 w-14 sm:w-20 rounded-md border bg-background px-2 text-xs sm:text-sm tabular-nums focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring ${hasQty ? "border-primary" : "border-input"}`}
                        />
                      </td>
