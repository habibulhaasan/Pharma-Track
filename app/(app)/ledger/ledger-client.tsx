...
export function LedgerClient({ products }: { products: Product[] }) {
  ...
  const [viewMode,    setViewMode]    = useState<ViewMode>(now.getMonth() + 1);
  ...
  const buildStatement = (): DayRow[] => {
    if (isFullMode) {
      ...
      Object.keys(monthMap).sort().forEach((key) => {
        ...
        rows.push({ dateKey: `Opening Balance (${label})`, sortKey: 0, in: 0, out: 0, balance: runningBal, descriptions: ["Opening Balance"], details: [] });
        ...
        mEntries.forEach((e) => {
          ...
          const delta = e.type === "ADJUSTMENT" ? (e.adjustmentDelta ?? 0) : e.type === "IN" ? e.quantity : -e.quantity;
        ...
        Object.entries(dayMap).sort((a, b) => a[1].sortKey - b[1].sortKey).forEach(([dk, day]) => {
          ...
          rows.push({
            dateKey: dk, sortKey: day.sortKey,
    ...
    const rows: DayRow[] = [];
    rows.push({
      dateKey: `Opening Balance (${MONTHS[(viewMode as number) - 1]} ${year})`,
      sortKey: 0, in: 0, out: 0, balance: openingBalance,
